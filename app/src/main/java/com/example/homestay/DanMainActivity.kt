...
import com.example.HomeStay.ui.HomePage.HomePageScreen
...
fun HomeStayApp(windowSizeClass: WindowSizeClass, authViewModel: AuthViewModel) {
    ...
    Surface(modifier = Modifier.fillMaxSize()) {
        NavHost(navController = navController,
            ...
            composable(route = HomeStayScreen.Home.name){
                when(windowSizeClass.widthSizeClass){
                    WindowWidthSizeClass.Compact -> {
                        //Phone layout
                        HomePageScreen(isTablet = false,
                            onProfileButtonClicked = { navController.navigate(HomeStayScreen.Profile.name) })
                    }
                    WindowWidthSizeClass.Medium,
                    WindowWidthSizeClass.Expanded -> {
                        //Tablet layout
                        HomePageScreen(isTablet = true,
                            onProfileButtonClicked = { navController.navigate(HomeStayScreen.Profile.name) })
                    }
                }
            }
